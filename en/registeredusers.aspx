   ...
   string fname = Request["reguser"];
               ...
               try
               {
                   ...
                   while (userReader.Read()) { 
                     %>
                         <tr><th style="width:150px;"><%Response.Write(userReader["fname"]);%></th><th style="width:150px"><%Response.Write(userReader["lname"]);%></th><td><%Response.Write(userReader["role"]);%></td><td style="width:30px"><button class="btn btn-link" onclick="$('#hidrow<%Response.Write(userReader["idno"]);%>').slideToggle();"><span class="glyphicon glyphicon-pencil"></span>edit</button></td><td id="disbut<%Response.Write(userReader["idno"]);%>" style="width:30px;">
                         <%if (userReader["status"].ToString() == "active")
                           {%>
                         <button style="color:red" class="btn btn-link" onclick="disableuser(<%Response.Write(userReader["idno"]);%>,'deactive')"><span class="glyphicon glyphicon-off"></span>Disable</button>
                         <%}
                           else
                           { %>
                          <button style="color:#23ff23" class="btn btn-link" onclick="disableuser(<%Response.Write(userReader["idno"]);%>,'active')"><span class="glyphicon glyphicon-check"></span>Enable</button>
                         <%} %>
                         </td></tr>
                         ...
                         <table class="table table-bordered table-condensed table-striped" border="1" style="width:80%;background-color:#eeeeee">
                          ...
                          <tr><td>Username: <%Response.Write(userReader["username"]);%></td></tr>
                          <tr><td><input type="password" class="form-control" id="newpass<%Response.Write(userReader["idno"]);%>" value="<%Response.Write(userReader["fname"]);%>"></td><tr>
                          <tr><td><input type="password" class="form-control" id="newcpass<%Response.Write(userReader["idno"]);%>" value="<%Response.Write(userReader["fname"]);%>"></td><tr>
                          <tr><td><button class="btn btn-primary" onclick="savechangesofuser(<%Response.Write(userReader["idno"]);%>)">save changes</button><span id="errormsg<%Response.Write(userReader["idno"]);%>"></span></td></tr>
                         </table></center>
                         </td>
                         </tr>
                     <%
      ...
      <%try
        ...
        %>
                          ...
                          <tr><td>Username: <%Response.Write(userReader["uname"]);%></td></tr>
                          <tr><td><input type="password" class="form-control" id="newaudpass<%Response.Write(userReader["orgId"]);%>" value="<%Response.Write(userReader["pass"]);%>"></td><tr>
                          <tr><td><input type="password" class="form-control" id="newaudcpass<%Response.Write(userReader["orgId"]);%>" value="<%Response.Write(userReader["pass"]);%>"></td><tr>
