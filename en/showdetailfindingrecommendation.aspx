...
<tr style="color:black"><th colspan="10" style="font-size:16px;">Detail of findings and recommendations</th></tr>
   ...
   try
   {
       ...
       while (findingreader.Read()) {
        ...
        <th style="width:7%"><b style="font-size:18px">Finding <%Response.Write(findingcounter++);%></b></th>
        <td>
        <%string findingname="";
          ...
          <span id="visf<%Response.Write(findingreader["findingId"]);%>">
           ...
           <p style="color:blue" onclick="$('#visf<%Response.Write(findingreader["findingId"]);%>').slideUp();$('#hidf<%Response.Write(findingreader["findingId"]);%>').slideDown();">...Show more</p>
          </span>
          <span id="hidf<%Response.Write(findingreader["findingId"]);%>" style="display:none">
           <%Response.Write(findingreader["findingName"]);%>
           <p style="color:blue" onclick="$('#visf<%Response.Write(findingreader["findingId"]);%>').slideDown();$('#hidf<%Response.Write(findingreader["findingId"]);%>').slideUp();">Show less</p>
          </span>

      
        </td>
       <td><b style="">Finding type:</b><br><%Response.Write(findingtype);%></td><td><%Response.Write("<b>Working paper reference:</b> <br>"+findingreader["pRef"]);%></td><td><%Response.Write("<b>Source of finding:</b><br>" + findingreader["source"]+"");%></td>
        <td><b>Auditee response</b><br></i>
        <%
         string resp="";
           if (findingreader["auditeeResp"].ToString().Length > 50)
           {
               ...
               resp = findingreader["auditeeResp"].ToString();
                 ...
                 <%Response.Write(resp);%>
                 <p style="color:Blue;"onclick="$('#visspan<%Response.Write(findingreader["findingId"]);%>').slideUp();$('#hidspan<%Response.Write(findingreader["findingId"]);%>').slideDown();">...Show more</p>
                 </span>
                 ...
                 <%  Response.Write(findingreader["auditeeResp"] + " <i style='color:blue'>date:" + findingreader["respDate"] + "<br>Response by: " + findingreader["respfname"] + " " + findingreader["resplname"]);
                      %>
                      <p style="color:Blue;"onclick="$('#visspan<%Response.Write(findingreader["findingId"]);%>').slideDown();$('#hidspan<%Response.Write(findingreader["findingId"]);%>').slideUp();">Show less</p>
            
                 </span>    
                 </td>    
      <td><b>Actual financial value (ETB.)</b><br><%Response.Write(double.Parse(findingreader["actualValue"].ToString()).ToString("#,0.00"));%></td><td><b>Extrapolated financial value( ETB.)</b><br><%Response.Write(double.Parse(findingreader["extraValue"].ToString()).ToString("#,0.00"));%></td><td><b>Recovered financial value (ETB.)</b><br><%Response.Write(totrecvalue.ToString("#,0.00"));%></td><td><b>Unrecovered financial value (ETB.)</b><br><%Response.Write((double.Parse(findingreader["actualValue"].ToString())-totrecvalue).ToString("#,0.00"));%></td></tr>
         <tr><td colspan="10"><center><table class="table table-responsive table-condensed table-bordered table-striped" style="width:98%;">
        <tr style="color:black;font-size:18px;"><th colspan="9">Recommendations</th></tr>
        <%
           con2.Open();
           ...
           while (recomreader.Read()) {
               ...
               {
            %>
            <th style="border-top:"><b style="font-size:18px">Recommendation<%Response.Write(recomcounter++);%></b></th><td style="border-top:"><%Response.Write(recomreader["recomName"]);%></td><td style="border-top:">Potential saving<br><%Response.Write(double.Parse(recomreader["potSaving"].ToString()).ToString("#,0.00"));%></td>
            <td style="border-top:;width:200px;"><u><b>Current status</b></u><br>
               NOT implemented
            </td>
            <%
               }
               ...
               { 
                   %>
            <th style="border-top:2"><b style="font-size:18px">Recommendation<%Response.Write(recomcounter++);%></b></th><td style="border-top:"><%Response.Write(recomreader["recomName"]);%></td><td style="border-top:">Potential saving<br><%Response.Write(recomreader["potSaving"]);%></td>
            <td style="width:200px;border-top:;"><u><b>Current status</b></u><br>
            ...
            <%
             ...
             %>
           <th style="border-top:"><b style="font-size:18px">Recommendation<%Response.Write(recomcounter++);%></b></th><td style="border-top:"><%Response.Write(recomreader["recomName"]);%></td><td style="border-top:">Potential saving<br><%Response.Write(recomreader["potSaving"]);%></td>
            <td style="border-top:;width:200px;"><u><b>Current status</b></u><br>
            Fully implemented
            </td>
            <%
            ...
            <b>Extent of status </b><br>
            <%Response.Write(recomreader["statusExtent"]);%>
            </td>
            </tr>
            <tr><td></td><td></td><td></td><td colspan="2"><button class="btn btn-primary" onclick="openstatusofrecom(<%Response.Write(recomreader["recomId"]);%>)">Click here to update status of recommendation</button></td></tr>
            ...
            <tr style="background-color:;color:black;font-size:18px;border-top:;border-bottom:;"><th colspan="2">Action plan</th></tr>
            <%
               con3.Open();
               ...
               while (actionplanreader.Read()) {
                   ...
                   {
                %>
                <tr><td><%Response.Write(actionplanreader["actionplan"]);%></td><td style="width:200px;background-color:">Action plan date:<br><b style="color:blue"><%Response.Write(actionplanreader["actiondate"]);%></b><span style="color:red">Missed the deadline</span></td></tr>
              
                <%
                   }
                   else { 
                    %>
                <tr><td><%Response.Write(actionplanreader["actionplan"]);%></td><td style="width:200px;background-color:">Action plan date:<br><b style="color:blue"><%Response.Write(actionplanreader["actiondate"]);%></b></td></tr>
                    <%
                   }
                   %>
                   <tr><td colspan="2">
                  <center><table class="table table-responsive table-condensed table-hover table-striped table-bordered" style="width:90%">
                  <tr><th>No.</th><th>Action taken</th><th>Recovered financial value(ETB.)</th><th>Action taken date</th></tr>
                   ...
                   if (actiontakencounter == 0) {
                       Response.Write("<tr><th colspan='4'>No action taken is recoreded</th></tr>");
                   }
                   con4.Close();
                   %>
                   </table></center></td></tr>
 ...
 <!--****************pop up window***************-->
    <div class="panel"style="position:fixed;top:200px;left:400px;width:600px;height:250px;display:none;z-index:10;border:1px solid #c2c9cb" id="recomstatuswind">
          ...
          <p style="color:black">Update status of recommendation</p>
          </div>
          <div class="panel-body" id="recomstatus" style="height:60%">
            
          </div>
          <div class="panel-footer" style=";height:20%;text-align:right;">
          <button class="btn-primary" onclick="changestatufofrecom(<%Response.Write(Request["auditId"]);%>)" style="width:100px">Save</button>
          <button class="btn-primary" style="color:white;width:100px" onclick="$('#recomstatuswind').slideUp();">Close</button>
          </div>
         </div>
