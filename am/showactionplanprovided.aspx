...
<li><a class="btn btn-link btn-sm"style="cursor:pointer;background-color:#c2c9cb;color:black;border-radius:0px;" onclick="printpdfactionplanprovided()" ><span class="glyphicon glyphicon-share"></span>አትም</a></li>
</ul>
<br>
<table class="table">
<tr><th style="background-color:#c2c9cb;color:black">የተግባር መርሃ ግብር እና የተወሰዱ እርምጃዎች ዝርዝር</th></tr>
</table>
<br>
        ...
        con2.Open();
          ...
          SqlDataReader recomreader = new SqlCommand("select * from actionPlan_Table where auditId=" + (int)auditidreader["auditId"], con2).ExecuteReader();
           while(recomreader.Read()){
            %>
            <tr style="border-top:2px solid #c3c9cb;"><th style="background-color:#c2c9cb;color:black;"><%Response.Write(recomcounter++);%></th><td style="width:27%">ተገምጋሚ ድርጅት<br><%Response.Write(auditee);%></td><td><u>የድርጊት መርሃ—ግብር</u><br><%Response.Write(recomreader["actionplan"]);%></td></tr>
            <tr><td colspan="3">
            ...
            <% 
               ...
               %>
               <tr><td style="width:10%">የተወሰደ እርምጃ <%Response.Write(++actiontakencounter);%></td><td style="width:50%"><br><%Response.Write(actiontakenreader["actiontaken"]);%></td><td>የተመለሰ የፋይናንስ እሴት(ETB.): <%Response.Write(actiontakenreader["recoveredvalue"]); %></td><td style="width:10%">ቀን:<%Response.Write(actiontakenreader["actiondate"]);%></td></tr>
               <%
               }   
               con3.Close();
               if (actiontakencounter == 0) { 
               %>
               <tr><th colspan="3" style="font-size:16px;">ምንም እርምጃ አልተወሰደም</th></tr>
               <%
               }
               %>
               </table></td></tr>
